...
import {createGame,ensureDiversity,tick,STEP,buy,canAddSpecies,feed,price,serialize,restore,clamp} from '../src/simulation.js';
import {createWorld} from '../src/scene.js';
import {SPECIES,MAX_RESIDENTS,speciesOf,sizeLabel} from '../src/species.js';
const $=id=>document.getElementById(id);
const SAVE_KEY='fishslop.recording.full-flow.v1';
let saved=null,storageAvailable=true;
try{saved=restore(localStorage.getItem(SAVE_KEY));}catch{storageAvailable=false;}
saved=null;
let game=createGame(),playing=false,paused=false,muted=true,audioContext=null,ambient=null;
...
const manualInput=()=>({forward:Number(keys.has('KeyW'))-Number(keys.has('KeyS')),strafe:Number(keys.has('KeyD'))-Number(keys.has('KeyA')),vertical:Number(keys.has('KeyE'))-Number(keys.has('KeyQ')),turn:Number(keys.has('ArrowLeft'))-Number(keys.has('ArrowRight')),pitch:Number(keys.has('ArrowUp'))-Number(keys.has('ArrowDown')),boost:keys.has('ShiftLeft')||keys.has('ShiftRight'),feed:keys.has('Space')});
function updateHUD(){
...
function frame(now){runDemo(now);const dt=Math.min((now-last)/1000,.1);last=now;
...
// Local recording rehearsal: use the normal simulation, economy, HUD and shop handlers.
// The schedule changes only control input; it never grants coins, teleports, or isolates species.
let demoStart=0,demoActions=new Set(),demoElapsed=0,winShownAt=null;
document.addEventListener('demo-recording-start',()=>{demoStart=performance.now();demoActions.clear();});
const input=()=>demoStart?demoInput():manualInput();
function once(at,fn){if(demoElapsed>=at&&!demoActions.has(at)){demoActions.add(at);fn();}}
function runDemo(now){if(!demoStart)return;demoElapsed=(now-demoStart)/1000;
 once(4,()=>$('start').click());
 if($('modal').open&&$('modal-title').textContent==='Look what you grew.'){
  winShownAt??=demoElapsed;if(demoElapsed-winShownAt>3)closeModal();
 }
 once(50,()=>$('dock-toggle').click());
 once(53,()=>{speciesSelect.value='3';selectSpecies();});
 once(57,()=>shopButtons[0].click());
 once(61,()=>{speciesSelect.value='9';selectSpecies();});
 once(65,()=>shopButtons[0].click());
 once(69,()=>shopButtons[1].click());
 once(72,()=>shopButtons[2].click());
 once(76,()=>$('dock-toggle').click());
 once(89,()=>$('pause').click());
 once(93,()=>$('resume').click());
}
function demoInput(){const t=demoElapsed;
 if(t<6||t>=50&&t<77||t>=89&&t<94)return {};
 if(t<18)return {forward:.15,turn:.035,feed:true};
 if(t<23)return {vertical:.22,strafe:.15,feed:true};
 if(t<28)return {vertical:-.22,turn:-.04,feed:true};
 if(t<35)return {strafe:-.18,turn:-.055,feed:true};
 if(t<50||t>=77&&t<85){
  const s=game.sub,coin=game.drops.toSorted((a,b)=>Math.hypot(a.x-s.x,a.y-s.y,a.z-s.z)-Math.hypot(b.x-s.x,b.y-s.y,b.z-s.z))[0];
  if(coin){const dx=coin.x-s.x,dz=coin.z-s.z;return {forward:clamp((-Math.sin(s.yaw)*dx-Math.cos(s.yaw)*dz)*.3,-.5,.5),strafe:clamp((Math.cos(s.yaw)*dx-Math.sin(s.yaw)*dz)*.3,-.5,.5),vertical:clamp((coin.y-s.y)*.3,-.4,.4),feed:true};}
  return {forward:.06,turn:.025,feed:true};
 }
 return {forward:.09,turn:.045,feed:true};
}
