...
test('fixed step simulation is deterministic and respects all tank walls',()=>{
  const a=createGame(10),b=createGame(10);run(a,90,{forward:1,strafe:1,vertical:1,feed:true});run(b,90,{forward:1,strafe:1,vertical:1,feed:true});assert.equal(serialize(a),serialize(b));assert.ok(a.sub.x<=BOUNDS.x&&a.sub.y<=BOUNDS.maxY&&a.sub.z>=-BOUNDS.z);assert.ok(a.food.length<=45&&a.drops.length<=100);
});
