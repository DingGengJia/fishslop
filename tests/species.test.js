...
test('diversity migration adds every new species once and preserves existing residents and economy',()=>{
 const g=createGame(),before=JSON.stringify(g.fish);g.coins=875;assert.equal(ensureDiversity(g),true);assert.equal(g.fish.length,29);assert.equal(JSON.stringify(g.fish.slice(0,5)),before);assert.equal(g.coins,875);for(const type of [3,4,5,6,7,8,9,10,11])assert.ok(g.fish.some(f=>f.type===type));assert.equal(ensureDiversity(g),false);assert.equal(g.fish.length,29);
...
test('species purchases charge the selected species and reject invalid identifiers atomically',()=>{
 ...
 for(const item of ['fish:12','fish:-1','fish:NaN','fish:01']){const coins=g.coins;assert.equal(buy(g,item),false);assert.equal(g.coins,coins);}
...
test('new species survive simulation/save roundtrip while drifting and swimming at distinct speeds',()=>{
 ...
 const bad=JSON.parse(serialize(g));bad.fish[0].type=12;assert.equal(restore(JSON.stringify(bad)),null);
});
...
test('version 6 saves receive only the new arrivals and full aquariums retain all existing animals',()=>{
 ...
 assert.equal(ensureDiversity(g),true);assert.equal(g.fish.length,44);assert.equal(JSON.stringify(g.fish.slice(0,32)),before);assert.equal(g.coins,1000);assert.ok(restore(serialize(g)));assert.equal(ensureDiversity(g),false);
});
